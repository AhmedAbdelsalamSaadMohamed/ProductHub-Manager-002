...
CREATE OR REPLACE PACKAGE BODY ph_sec_management_validation_pkg AS
    ...
    FUNCTION user_has_permission(p_user_id IN NUMBER, p_object_name IN VARCHAR2, p_action_name IN VARCHAR2) RETURN NUMBER IS
        l_count NUMBER(10);
    BEGIN
        SELECT COUNT(*)
          INTO l_count
    ...
    FUNCTION permission_is_valid(p_user_id IN NUMBER, p_object_name IN VARCHAR2, p_action_name IN VARCHAR2, o_is_valid OUT NUMBER, o_validation_message OUT VARCHAR2) RETURN NUMBER IS
        l_user_id NUMBER;
    BEGIN
        l_user_id := COALESCE(p_user_id, ph_sec_authentication_pkg.get_user_id);

        IF l_user_id IS NULL THEN
            set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('User is required for permission validation.', 'User is required for permission validation.'));
            RETURN 0;
        ELSIF user_has_permission(l_user_id, p_object_name, p_action_name) = 0 THEN
            set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('User does not have permission to perform this action.', 'User does not have permission to perform this action.'));
            RETURN 0;
        END IF;
    ...
    BEGIN
        IF permission_is_valid(p_created_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN RETURN; END IF;
    ...
    BEGIN
        IF permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN RETURN; END IF;
    ...
    BEGIN
        IF permission_is_valid(p_assigned_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN RETURN; END IF;
        IF exists_user(p_user_id, 1) = 0 THEN set_invalid(o_is_valid, o_validation_message, 'Active user was not found.');
    ...
    BEGIN
        IF permission_is_valid(NULL, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN RETURN; END IF;
