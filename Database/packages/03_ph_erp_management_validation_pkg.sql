...
CREATE OR REPLACE PACKAGE BODY ph_erp_management_validation_pkg AS
    ...
    BEGIN
        IF permission_is_valid(NULL, 'PLATFORM_SETUP', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN RETURN; END IF;
    ...
    BEGIN
        IF permission_is_valid(p_created_by, 'PRODUCTS', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN RETURN; END IF;
    ...
    BEGIN
        IF permission_is_valid(p_updated_by, 'PRODUCTS', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN RETURN; END IF;
    ...
    BEGIN
        IF permission_is_valid(p_updated_by, 'PRODUCTS', 'DELETE', o_is_valid, o_validation_message) = 0 THEN RETURN; END IF;
        IF exists_product(p_product_id) = 0 THEN
            set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Product was not found.', 'Product was not found.'));
        ELSE
            set_valid(o_is_valid, o_validation_message);
