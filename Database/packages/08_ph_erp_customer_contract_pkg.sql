...
CREATE OR REPLACE PACKAGE ph_erp_customer_contract_pkg AS
    ...
    ----------------------------------------------------------------------
    FUNCTION is_active_customer(p_customer_id IN NUMBER) RETURN NUMBER;
    FUNCTION has_initial_admin(p_customer_id IN NUMBER) RETURN NUMBER;
    FUNCTION has_active_contract(p_customer_id IN NUMBER) RETURN NUMBER;
    FUNCTION get_contract_customer_id(p_contract_id IN NUMBER) RETURN NUMBER;
    FUNCTION get_contract_product_id(p_contract_id IN NUMBER) RETURN NUMBER;
    FUNCTION is_module_valid_for_contract(p_contract_id IN NUMBER, p_product_id IN NUMBER, p_module_id IN NUMBER) RETURN NUMBER;
    FUNCTION is_feature_valid_for_contract(p_contract_id IN NUMBER, p_product_id IN NUMBER, p_module_id IN NUMBER, p_platform_id IN NUMBER, p_feature_id IN NUMBER) RETURN NUMBER;
    FUNCTION has_primary_url(p_contract_id IN NUMBER) RETURN NUMBER;

    PROCEDURE require_active_customer(p_customer_id IN NUMBER);
    PROCEDURE require_valid_contract_module(p_contract_id IN NUMBER, p_product_id IN NUMBER, p_module_id IN NUMBER);
    PROCEDURE require_valid_contract_feature(p_contract_id IN NUMBER, p_product_id IN NUMBER, p_module_id IN NUMBER, p_platform_id IN NUMBER, p_feature_id IN NUMBER);
...
CREATE OR REPLACE PACKAGE BODY ph_erp_customer_contract_pkg AS
    ...
    ----------------------------------------------------------------------
    -- Public validation and lookup implementations
    ----------------------------------------------------------------------
    ...
    BEGIN
        IF NVL(p_value, -1) NOT IN (0, 1) THEN
            RAISE_APPLICATION_ERROR(-20290, ph_localization_pkg.localized_text(p_name || ' must be 0 or 1.', p_name || ' يجب أن يكون 0 أو 1.'));
        END IF;
    END require_flag;
    ...
    BEGIN
        IF TRIM(p_value) IS NULL THEN
            RAISE_APPLICATION_ERROR(-20291, ph_localization_pkg.localized_text(p_name || ' is required.', p_name || ' مطلوب.'));
        END IF;
    END require_text;
    ...
    BEGIN
        IF p_start_date IS NULL THEN
            RAISE_APPLICATION_ERROR(-20292, ph_localization_pkg.localized_text('Contract start date is required.', 'تاريخ بداية العقد مطلوب.'));
        END IF;

        IF p_end_date IS NOT NULL AND p_end_date < p_start_date THEN
            RAISE_APPLICATION_ERROR(-20293, ph_localization_pkg.localized_text('Contract end date cannot be before start date.', 'لا يمكن أن يكون تاريخ نهاية العقد قبل تاريخ البداية.'));
        END IF;
    ...
    BEGIN
        IF p_effective_from IS NULL THEN
            RAISE_APPLICATION_ERROR(-20294, ph_localization_pkg.localized_text('Effective from date is required.', 'تاريخ بداية السريان مطلوب.'));
        END IF;

        IF p_effective_to IS NOT NULL AND p_effective_to < p_effective_from THEN
            RAISE_APPLICATION_ERROR(-20295, ph_localization_pkg.localized_text('Effective to date cannot be before effective from date.', 'لا يمكن أن يكون تاريخ نهاية السريان قبل تاريخ البداية.'));
        END IF;
    ...
    BEGIN
        ...
        IF l_exists = 0 THEN
            RAISE_APPLICATION_ERROR(-20210, ph_localization_pkg.localized_text('Customer was not found.', 'لم يتم العثور على العميل.'));
        END IF;
    ...
    BEGIN
        ...
        SELECT COUNT(*) INTO l_exists
         ...
         WHERE user_id = p_user_id
           ...
           AND is_deleted = 0;
        IF l_exists = 0 THEN
            RAISE_APPLICATION_ERROR(-20220, ph_localization_pkg.localized_text('Customer user was not found.', 'لم يتم العثور على مستخدم العميل.'));
        END IF;
    ...
    BEGIN
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                RAISE_APPLICATION_ERROR(-20211, ph_localization_pkg.localized_text('Contract was not found.', 'لم يتم العثور على العقد.'));
        END;
    ...
    BEGIN
        ...
        IF l_exists = 0 THEN
            RAISE_APPLICATION_ERROR(-20212, ph_localization_pkg.localized_text('Contract URL was not found.', 'لم يتم العثور على رابط العقد.'));
        END IF;
    ...
    BEGIN
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                RAISE_APPLICATION_ERROR(-20213, ph_localization_pkg.localized_text('Contract module was not found.', 'لم يتم العثور على وحدة العقد.'));
        END;
    ...
    BEGIN
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                RAISE_APPLICATION_ERROR(-20214, ph_localization_pkg.localized_text('Contract feature was not found.', 'لم يتم العثور على ميزة العقد.'));
        END;
    ...
    EXCEPTION WHEN OTHERS THEN set_error(p_result_code, p_result_message);
    END create_customer;
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RAISE_APPLICATION_ERROR(-20200, ph_localization_pkg.localized_text('Contract URL was not found.', 'لم يتم العثور على رابط العقد.'));
    END set_primary_url;
    ...
    BEGIN
        IF is_active_customer(p_customer_id) = 0 THEN
            RAISE_APPLICATION_ERROR(-20201, ph_localization_pkg.localized_text('Customer is not active or was not found.', 'العميل غير نشط أو غير موجود.'));
        END IF;
    ...
    BEGIN
        IF is_module_valid_for_contract(p_contract_id, p_product_id, p_module_id) = 0 THEN
            RAISE_APPLICATION_ERROR(-20202, ph_localization_pkg.localized_text('Module is not valid for the selected contract.', 'الوحدة غير صالحة للعقد المحدد.'));
        END IF;
    ...
    BEGIN
        IF is_feature_valid_for_contract(p_contract_id, p_product_id, p_module_id, p_platform_id, p_feature_id) = 0 THEN
            RAISE_APPLICATION_ERROR(-20203, ph_localization_pkg.localized_text('Feature is not valid for the selected contract module.', 'الميزة غير صالحة لوحدة العقد المحددة.'));
        END IF;
