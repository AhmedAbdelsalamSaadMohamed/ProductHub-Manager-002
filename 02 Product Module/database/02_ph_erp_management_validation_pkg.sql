...
CREATE OR REPLACE PACKAGE BODY ph_erp_management_validation_pkg AS
    ...
    BEGIN
        IF ph_helpers_pkg.text_missing(p_name_en) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text(p_name_label_en || ' English name is required.', p_name_label_en || ' English name is required.'));
        ELSIF ph_helpers_pkg.text_missing(p_name_ar) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text(p_name_label_ar || ' Arabic name is required.', p_name_label_ar || ' Arabic name is required.'));
        ELSIF ph_helpers_pkg.text_too_long(p_name_en, p_max_length) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text(p_name_label_en || ' English name must not exceed ' || p_max_length || ' characters.', p_name_label_en || ' English name must not exceed ' || p_max_length || ' characters.'));
        ELSIF ph_helpers_pkg.text_too_long(p_name_ar, p_max_length) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text(p_name_label_ar || ' Arabic name must not exceed ' || p_max_length || ' characters.', p_name_label_ar || ' Arabic name must not exceed ' || p_max_length || ' characters.'));
        ELSE
    ...
    BEGIN
        IF ph_helpers_pkg.text_too_long(p_name_en, p_max_length) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text(p_name_label_en || ' English name must not exceed ' || p_max_length || ' characters.', p_name_label_en || ' English name must not exceed ' || p_max_length || ' characters.'));
        ELSIF ph_helpers_pkg.text_too_long(p_name_ar, p_max_length) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text(p_name_label_ar || ' Arabic name must not exceed ' || p_max_length || ' characters.', p_name_label_ar || ' Arabic name must not exceed ' || p_max_length || ' characters.'));
        ELSE
    ...
    BEGIN
        ...
        IF o_is_valid = 1 AND duplicate_platform_name(p_platform_name_en, p_platform_name_ar) = 1 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Platform name already exists.', 'Platform name already exists.'));
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
    ...
    BEGIN
        ...
        IF exists_platform(p_platform_id) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Platform was not found.', 'Platform was not found.'));
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_active) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Active flag must be 0 or 1.', 'Active flag must be 0 or 1.'));
        ELSE
    ...
    BEGIN
        ...
        IF exists_pricing_unit(p_pricing_unit_id) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Pricing unit was not found.', 'Pricing unit was not found.'));
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_active) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Active flag must be 0 or 1.', 'Active flag must be 0 or 1.'));
        ELSE
    ...
    BEGIN
        ...
        IF o_is_valid = 1 AND NVL(p_months_count, 0) <= 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Payment cycle months count must be greater than zero.', 'Payment cycle months count must be greater than zero.'));
        END IF;
    ...
    BEGIN
        ...
        IF exists_payment_cycle(p_payment_cycle_id) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Payment cycle was not found.', 'Payment cycle was not found.'));
    ...
    BEGIN
        ...
        IF o_is_valid = 1 AND (ph_helpers_pkg.text_too_long(p_description_en, 1000) OR ph_helpers_pkg.text_too_long(p_description_ar, 1000)) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Product description must not exceed 1000 characters.', 'Product description must not exceed 1000 characters.'));
        END IF;
    ...
    BEGIN
        ...
        IF exists_product(p_product_id) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Product was not found.', 'Product was not found.'));
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_active) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Active flag must be 0 or 1.', 'Active flag must be 0 or 1.'));
        ELSE
    ...
    BEGIN
        ...
        IF exists_product(p_product_id, 1) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Product is not active or was not found.', 'Product is not active or was not found.'));
        ELSE
            validate_name_pair(p_module_name_en, p_module_name_ar, 'Module', 'Module', 200, o_is_valid, o_validation_message);
            IF o_is_valid = 1 AND (ph_helpers_pkg.text_too_long(p_description_en, 1000) OR ph_helpers_pkg.text_too_long(p_description_ar, 1000)) THEN
                ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Module description must not exceed 1000 characters.', 'Module description must not exceed 1000 characters.'));
            END IF;
        END IF;
    ...
    BEGIN
        ...
        IF exists_module(p_product_id, p_module_id) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Module was not found.', 'Module was not found.'));
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_active) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Active flag must be 0 or 1.', 'Active flag must be 0 or 1.'));
        ELSE
    ...
    BEGIN
        ...
        IF exists_module(p_product_id, p_module_id, 1) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Module is not active for the selected product.', 'Module is not active for the selected product.'));
        ELSIF active_platform(p_platform_id) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Platform is not active or was not found.', 'Platform is not active or was not found.'));
        ELSE
            ph_helpers_pkg.set_valid(o_is_valid, o_validation_message);
        END IF;
    ...
    BEGIN
        ...
        IF exists_module_platform(p_product_id, p_module_id, p_platform_id) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Module platform was not found.', 'Module platform was not found.'));
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_active) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Active flag must be 0 or 1.', 'Active flag must be 0 or 1.'));
        ELSE
            ph_helpers_pkg.set_valid(o_is_valid, o_validation_message);
    ...
    BEGIN
        ...
        IF exists_module_platform(p_product_id, p_module_id, p_platform_id, 1) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Module platform is not active for the selected product and module.', 'Module platform is not active for the selected product and module.'));
        ELSIF NVL(p_price, -1) < 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Feature price must be greater than or equal to zero.', 'Feature price must be greater than or equal to zero.'));
        ELSIF active_pricing_unit(p_pricing_unit_id) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Pricing unit is not active or was not found.', 'Pricing unit is not active or was not found.'));
        ELSE
            validate_name_pair(p_feature_name_en, p_feature_name_ar, 'Feature', 'Feature', 200, o_is_valid, o_validation_message);
            IF o_is_valid = 1 AND (ph_helpers_pkg.text_too_long(p_description_en, 1000) OR ph_helpers_pkg.text_too_long(p_description_ar, 1000) OR ph_helpers_pkg.text_too_long(p_usage_unit, 50)) THEN
                ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Feature text values exceed the allowed length.', 'Feature text values exceed the allowed length.'));
            END IF;
        END IF;
    ...
    BEGIN
        ...
        IF exists_feature(p_product_id, p_module_id, p_platform_id, p_feature_id) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_text('Feature was not found.', 'Feature was not found.'));
