...
CREATE OR REPLACE PACKAGE BODY ph_sec_management_validation_pkg AS
    ...
    BEGIN
        IF ph_helpers_pkg.text_missing(p_name_en) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, p_label || ' English name is required.');
        ELSIF ph_helpers_pkg.text_missing(p_name_ar) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, p_label || ' Arabic name is required.');
        ELSIF ph_helpers_pkg.text_too_long(p_name_en, p_max_length) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, p_label || ' English name must not exceed ' || p_max_length || ' characters.');
        ELSIF ph_helpers_pkg.text_too_long(p_name_ar, p_max_length) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, p_label || ' Arabic name must not exceed ' || p_max_length || ' characters.');
        ELSE
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_user_type(p_user_type_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'User type was not found.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_active) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'User type active flag must be 0 or 1.');
        ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_user_type(p_user_type_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'User type was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_user_type(p_user_type_id, 0, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'User type was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_object_type(p_object_type_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Object type was not found.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_active) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Object type active flag must be 0 or 1.');
        ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_object_type(p_object_type_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Object type was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_object_type(p_object_type_id, 0, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Object type was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_created_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF ph_helpers_pkg.text_missing(p_object_name) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Object name is required.');
        ELSIF exists_object_type(p_object_type_id, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active object type was not found.');
        ELSIF p_parent_object_id IS NOT NULL AND exists_object(p_parent_object_id, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active parent object was not found.');
        ELSIF ph_helpers_pkg.text_missing(p_object_path) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Object path is required.');
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_object(p_object_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Object was not found.');
        ELSIF p_parent_object_id IS NOT NULL AND (p_parent_object_id = p_object_id OR exists_object(p_parent_object_id, 1) = 0) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active parent object was not found or is invalid.');
        ELSIF p_object_type_id IS NOT NULL AND exists_object_type(p_object_type_id, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active object type was not found.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_active) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Object active flag must be 0 or 1.');
        ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_object(p_object_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Object was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_object(p_object_id, 0, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Object was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_created_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF ph_helpers_pkg.text_missing(p_action_name) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Action name is required.');
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_action(p_action_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Action was not found.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_active) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Action active flag must be 0 or 1.');
        ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_action(p_action_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Action was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_action(p_action_id, 0, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Action was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_created_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_object(p_object_id, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active object was not found.');
        ELSIF exists_action(p_action_id, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active action was not found.');
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_permission(p_permission_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Permission was not found.');
        ELSIF p_object_id IS NOT NULL AND exists_object(p_object_id, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active object was not found.');
        ELSIF p_action_id IS NOT NULL AND exists_action(p_action_id, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active action was not found.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_active) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Permission active flag must be 0 or 1.');
        ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_permission(p_permission_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Permission was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_permission(p_permission_id, 0, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Permission was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_created_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF ph_helpers_pkg.text_missing(p_email) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'User email is required.');
        ELSIF ph_helpers_pkg.text_missing(p_display_name) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'User display name is required.');
        ELSIF exists_user_type(p_user_type, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active user type was not found.');
        ELSIF exists_customer(p_customer_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active customer was not found.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_must_change_password) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Must change password flag must be 0 or 1.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_initial_admin) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Initial admin flag must be 0 or 1.');
        ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_user(p_user_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'User was not found.');
        ELSIF p_user_type IS NOT NULL AND exists_user_type(p_user_type, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active user type was not found.');
        ELSIF exists_customer(p_customer_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active customer was not found.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_must_change_password) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Must change password flag must be 0 or 1.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_initial_admin) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Initial admin flag must be 0 or 1.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_active) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'User active flag must be 0 or 1.');
        ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_user(p_user_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'User was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_user(p_user_id, 0, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'User was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_created_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_user(p_user_id, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active user was not found.');
        ELSIF ph_helpers_pkg.text_missing(p_preference_code) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Preference code is required.');
        ELSIF NOT ph_helpers_pkg.valid_value_type(COALESCE(p_value_type, 'STRING')) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Preference value type must be STRING, NUMBER, BOOLEAN, or JSON.');
        ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_user_preference(p_user_id, p_preference_code) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'User preference was not found.');
        ELSIF NOT ph_helpers_pkg.valid_value_type(p_value_type) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Preference value type must be STRING, NUMBER, BOOLEAN, or JSON.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_active) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'User preference active flag must be 0 or 1.');
        ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_user_preference(p_user_id, p_preference_code) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'User preference was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_user_preference(p_user_id, p_preference_code, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'User preference was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        ...
        IF o_is_valid = 0 THEN
            RETURN;
        END IF;
        IF exists_user_type(p_user_type, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active user type was not found.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_system_role) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'System role flag must be 0 or 1.');
        ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_role(p_role_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Role was not found.');
        ELSIF p_user_type IS NOT NULL AND exists_user_type(p_user_type, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active user type was not found.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_system_role) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'System role flag must be 0 or 1.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_active) THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Role active flag must be 0 or 1.');
        ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_role(p_role_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Role was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_role(p_role_id, 0, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Role was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_created_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_role(p_role_id, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active role was not found.');
        ELSIF exists_permission(p_permission_id, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active permission was not found.');
        ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_role_permission(p_role_id, p_permission_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Role permission was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_role_permission(p_role_id, p_permission_id, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Role permission was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        ...
        END IF;
        IF exists_user(p_user_id, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active user was not found.');
        ELSIF exists_role(p_role_id, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Active role was not found.');
        ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_user_role(p_user_id, p_role_id) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'User role was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_updated_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF exists_user_role(p_user_id, p_role_id, 1) = 0 THEN ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'User role was not found.'); ELSE ph_helpers_pkg.set_valid(o_is_valid, o_validation_message); END IF;
    ...
    BEGIN
        IF ph_sec_authorization_pkg.permission_is_valid(p_created_by, 'SECURITY_ADMIN', 'MANAGE', o_is_valid, o_validation_message) = 0 THEN
            RETURN;
        END IF;
        IF ph_helpers_pkg.text_missing(p_page_type_code) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page type code is required.');
        ELSIF ph_helpers_pkg.text_too_long(p_page_type_code, 50) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page type code must not exceed 50 characters.');
        ELSIF duplicate_apex_page_type_code(p_page_type_code) = 1 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page type code already exists.');
        ELSE
    ...
    BEGIN
        ...
        IF exists_apex_page_type(p_page_type_id) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_message('APEX_PAGE_TYPE_NOT_FOUND'));
        ELSIF p_page_type_code IS NOT NULL AND ph_helpers_pkg.text_missing(p_page_type_code) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page type code is required.');
        ELSIF ph_helpers_pkg.text_too_long(p_page_type_code, 50) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page type code must not exceed 50 characters.');
        ELSIF duplicate_apex_page_type_code(p_page_type_code, p_page_type_id) = 1 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page type code already exists.');
        ELSIF p_page_type_name_en IS NOT NULL AND ph_helpers_pkg.text_missing(p_page_type_name_en) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page type English name is required.');
        ELSIF p_page_type_name_ar IS NOT NULL AND ph_helpers_pkg.text_missing(p_page_type_name_ar) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page type Arabic name is required.');
        ELSIF ph_helpers_pkg.text_too_long(p_page_type_name_en, 100) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page type English name must not exceed 100 characters.');
        ELSIF ph_helpers_pkg.text_too_long(p_page_type_name_ar, 100) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page type Arabic name must not exceed 100 characters.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_active) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page type active flag must be 0 or 1.');
        ELSE
            ph_helpers_pkg.set_valid(o_is_valid, o_validation_message);
        END IF;
    ...
    BEGIN
        ...
        IF p_apex_app_id IS NULL OR p_apex_app_id <= 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX app id must be greater than zero.');
        ELSIF p_apex_page_no IS NULL OR p_apex_page_no < 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page number must be greater than or equal to zero.');
        ELSIF exists_apex_page_type(p_apex_page_type_id, 1) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page type is not active or was not found.');
        ELSIF ph_helpers_pkg.text_too_long(p_page_alias, 128) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page alias must not exceed 128 characters.');
        ELSIF ph_helpers_pkg.text_too_long(p_object_path, 300) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page object path must not exceed 300 characters.');
        ELSIF NOT valid_access_mode(p_access_mode) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page access mode must be ANY or ALL.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_public) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page public flag must be 0 or 1.');
        ELSIF duplicate_apex_page_no(p_apex_app_id, p_apex_page_no) = 1 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX app page number already exists.');
        ELSIF duplicate_apex_page_alias(p_apex_app_id, p_page_alias) = 1 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page alias already exists for this app.');
        ELSE
    ...
    BEGIN
        ...
        IF exists_apex_page(p_apex_page_id) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_message('APEX_PAGE_NOT_FOUND'));
            RETURN;
        END IF;
        ...
        ELSIF p_apex_page_type_id IS NOT NULL AND exists_apex_page_type(p_apex_page_type_id, 1) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page type is not active or was not found.');
        ELSIF p_page_name_en IS NOT NULL AND ph_helpers_pkg.text_missing(p_page_name_en) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page English name is required.');
        ELSIF p_page_name_ar IS NOT NULL AND ph_helpers_pkg.text_missing(p_page_name_ar) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page Arabic name is required.');
        ELSIF ph_helpers_pkg.text_too_long(p_page_name_en, 200) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page English name must not exceed 200 characters.');
        ELSIF ph_helpers_pkg.text_too_long(p_page_name_ar, 200) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page Arabic name must not exceed 200 characters.');
        ELSIF ph_helpers_pkg.text_too_long(p_page_alias, 128) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page alias must not exceed 128 characters.');
        ELSIF ph_helpers_pkg.text_too_long(p_object_path, 300) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page object path must not exceed 300 characters.');
        ...
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_public) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page public flag must be 0 or 1.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_active) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page active flag must be 0 or 1.');
    ...
    BEGIN
        ...
        IF exists_apex_page(p_apex_page_id, 1) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page is not active or was not found.');
        ELSIF exists_permission(p_permission_id, 1) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'Permission is not active or was not found.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_an_access_permission) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page access permission flag must be 0 or 1.');
        ELSE
            ph_helpers_pkg.set_valid(o_is_valid, o_validation_message);
        END IF;
    ...
    BEGIN
        ...
        IF exists_apex_page_permission(p_apex_page_id, p_permission_id) = 0 THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, ph_localization_pkg.localized_message('APEX_PAGE_PERMISSION_NOT_FOUND'));
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_an_access_permission) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page access permission flag must be 0 or 1.');
        ELSIF NOT ph_helpers_pkg.valid_flag(p_is_active) THEN
            ph_helpers_pkg.set_invalid(o_is_valid, o_validation_message, 'APEX page permission active flag must be 0 or 1.');
        ELSE
            ph_helpers_pkg.set_valid(o_is_valid, o_validation_message);
        END IF;
